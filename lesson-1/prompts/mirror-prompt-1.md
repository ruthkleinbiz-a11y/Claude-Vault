# Mirror Prompt 1 (formerly known as God Prompt 1) — Claude Conversion

**Original:** "3-Mirror Prompt 1_V2e" from Prompts Edit > Lesson 1
...
- Run the Mirror Prompt after reaching a Zenith Mirror score of 80+
...
- When you're ready to go even deeper, run Mirror Prompt 2
...
- **Save output to** `outputs/mirror-prompt-1-output.md`
- **After saving**, transition to the Mirror Prompt Action Guide reflection flow (see `prompts/mirror-prompt-action-guide.md`)
- **Save reflections to** `outputs/mirror-prompt-1-reflections.md`
- **Update** `outputs/psych-profile.md` with new insights

### Output structure:

The Mirror Prompt 1 report IS the user's first "Hidden Obstacles to Success Report" (see `prompts/hidden-obstacles-report.md`). It should follow the report structure:
...
**Section delivery — Fluid, Not Interruptive (matches orchestrator):** Do not generate the entire report at once. Present sections in natural groups (2-3 related sections together). Check in at the midpoint — roughly halfway through, pause and ask "How's this landing so far?" If the user is clearly engaged, let them drive the pace. If quiet, check in more frequently. User can always say "keep going" or "pause." Save the complete report only after all sections are delivered and the user has had a chance to react.

1. Check Zenith Mirror Score ≥ 80
2. Read all `outputs/` files for full context
3. Generate report in natural groups — Hidden Narrative + Unexpressed Fear together, then Recursive Unpacking + Triggers, then Pattern Detection + Pareto + Ultimate Truth. Check in at midpoint.
4. Apply Fluff & Cliche Killer to each section before presenting it
5. After all sections delivered and user has reacted, save complete validated report to `outputs/mirror-prompt-1-output.md`
7. Run Mirror Prompt Action Guide reflection (Phases 1-4) — focused on deeper processing since accuracy was confirmed section by section
8. Save reflections to `outputs/mirror-prompt-1-reflections.md`
9. Update `outputs/psych-profile.md`
10. Tell user: "When you're ready for deeper analysis, we'll run Mirror Prompt 2."
